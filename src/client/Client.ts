...
import { ClientOptions, DefaultClientOptions, resolveIntents } from './ClientOptions';
...
export class Client extends EventEmitter {
  ...
  public constructor(token: string, options?: ClientOptions) {
    ...
    this.options = Object.assign(DefaultClientOptions, options) as Required<ClientOptions>;
    this.options.intents = resolveIntents(this.options.intents!!);
    this.#token = token;
    this.http = new RequestManager(
      new RESTClient(RESTClient.createRESTOptions(this.options, this.#token, 'Bot')), this,
    );
  }
  ...
  public async connect(): Promise<void> {
    ...
    if (gatewayInfo.shards > 1 && !process.env.__FUWA_SHARD_COUNT) throw new Error("Discord has recommended to use shards for this user but no shard information was found. Consider using a ShardingManager.")
    
    const url = this.constructGatewayURL(gatewayInfo.url);
    const shard: [id: number, total: number] = [+(process.env.__FUWA_SHARD_ID ?? 0), +(gatewayInfo.shards ?? 1)];
...
`.trim()
  ...
  private constructGatewayURL(url: string) {
    ...
    return `${url}?v=${this.options.apiVersion}&encoding=${this.options.etf ? "etf" : "json"}`
  }
