...
export function resolveIntents(intents: ClientOptionsIntents): Intents {
  ...
  // @ts-ignore aaaaaa
  if (Array.isArray(intents)) return new Intents(intents.reduce((prev: number, next) => prev + <number>((<Intents>next).bits ?? next), 0) as number);
  throw new TypeError("Client intents must be a number, an array of numbers or Intents, or an Intents instance.");
}
