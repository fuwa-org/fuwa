...
import { GatewayIdentify, GatewayOpcodes, GatewayResume, GatewaySendPayload, GatewayCloseCodes, GatewayDispatchPayload } from "@splatterxl/discord-api-types";
...
export class GatewayShard {
  ...
  constructor(public client: Client, public readonly shard: [number, number], token: string) {
    this.id = shard[0]!;
  ...
  public async connect(url: string) {
    ...
    this._socket.on("close", (code, reason) => {
      ...
      switch (code) {
        case 1000:
          throw new Error(`Gateway closed with code 1000: ` + reason.toString())
        case GatewayCloseCodes.InvalidIntents: 
          throw new Error(`Gateway intents ${this.client.options.intents} are invalid.`);
        default: 
  ...
  private onMessage(message: Buffer) {
    ...
    const data: GatewayDispatchPayload = this.erlpack ? buffer as unknown as any : JSON.parse(buffer.toString());
  ...
  public async send(packet: GatewaySendPayload) {
    if (!this._socket) throw new Error("GatewayShard#send called when shard wasn't connected");
