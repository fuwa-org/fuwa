...
export class Message<ChannelType extends BaseTextChannel> extends BaseStructure<APIMessage> {
  public id!: Snowflake;
  ...
  _deserialise(data: APIMessage): this {
    ...
    if ("guild_id" in data) this.guild = this.client.guilds.resolve(data.guild_id) ?? null;
    if ("channel_id" in data) this.channel = this.guild?.channels.resolve(data.channel_id)! as unknown as ChannelType ?? null;
    ...
    if ("member" in data) this.member = this.guild!.members.resolve(data.member!) ?? null;
  ...
  _modify(data: Partial<APIMessage>): Promise<Message<ChannelType>> {
    return this.client.http.queue<APIMessage>({
      route: Routes.channelMessage(this.channel!.id, this.id),
      method: "PATCH",
      body: DataTransformer.asJSON(data),
    }).then(async data => <this>this.channel!.messages.update(this._deserialise(await data.body.json()))!)
  }
