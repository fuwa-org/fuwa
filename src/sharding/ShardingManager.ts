...
export class ShardingManager {
  ...
  public async spawn() {
    this.gatewayInfo = await this.client.execute(resolveRequest({ route: Routes.gatewayBot() })).then(consumeJSON);
  ...
  private spawnProcess(id: number) {
    ...
    worker.on("exit", code => {
      if (code !== 0) {
        console.error(`[ShardingManager] Worker ${id} exited with code ${code}, respawning`);
        this.respawn(id);
      } else {
        console.log(`[ShardingManager] Worker ${id} exited with success code ${code}`);
      }
    });
